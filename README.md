...
>
> `D1M` is decoded field by field, but a few groups stay constant across every
> capture and are therefore still unexplained, and the VFO B half is passed
> through verbatim. No other radio model has been tried yet. Expect rough
> edges.
>
...
FTM-400D ──CT-174──▶ HRI-200 ──USB──▶ Raspberry Pi
                                       │
                     /dev/ttyACM0 ─────┤────▶ hri200node.py ──▶ web panel :8080
                     (control)         │        │
...
- **The node owns only `/dev/ttyACM0`.** It performs the handshake, runs the
  poll loop, sets the channel configuration at startup, and converts PTT and
  COS.
- **Changing a radio setting needs no restart.** The box only reads its channel
  configuration during initialisation, so the node closes the port, repeats the
  handshake and sends a new frame — about four seconds, and SvxLink never
  notices.

A useful safety property falls out of the protocol: **PTT is asserted by what
you poll with, not by a latching command.** If the node crashes, stalls or is
...
Put the radio into node mode and plug the box in **first**: power the FTM-400D
on while holding `[D/X]` + `[GM]` until the display reads `HRI-200`. `[D/X]`
alone gives PDN mode, which looks similar and does not work.
...
That is all of it. The installer handles packages, sound files, ALSA, mixer
levels, `svxlink.conf`, systemd, the firewall and automatic updates, then
prints a summary and a health report.

Everything else — callsign, EchoLink, frequency, power, tone — is set in the
web panel it puts on port 8080, so the same image works for anybody.

**[INSTALL.md](INSTALL.md)** covers it properly, including a troubleshooting
table of every trap this cost an evening to find.
...
```bash
hri200node.py --check         what is wrong, and why
hri200node.py --setup         patch svxlink.conf and /etc/asound.conf
hri200node.py --wait-network  block until DNS answers, used by systemd
```
...
D1M....     full channel config — frequency, mode, narrow, tone, CTCSS, DCS,
            power. Read only at initialisation; the host sets it every startup
...
Every field in `D1M` was found by capturing one WIRES-X session per setting and
diffing: each changed exactly one character, so nothing had to be guessed.
...
protection. So `sudo ./hri200node.py` fails with `EACCES` where `sudo -u svxlink
./hri200node.py` succeeds. Use the systemd unit and the question does not arise.
...
- Decode what remains in `D1M` — three 3-character groups and two single
  characters are still constant `000`/`0` across every capture taken so far
