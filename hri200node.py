...
# Ranges are fixed by the codec: Speaker 0-47, PCM 0-55.
#
# Different radios present different audio levels on the data connector and
# want different drive, so a working setting for one is not one for another.
# These are starting points to measure from, not gospel - see INSTALL.md.
# "controllable" says whether the radio answers D1V0000 and can therefore be
# tuned by the host. It only tunes how long detection waits - the answer from
# the radio is always what decides, so a swapped radio corrects itself.
RADIO_PROFILES = {
    "ftm400d": {"label": "Yaesu FTM-400D", "speaker": 47, "pcm": 45,
                "controllable": True},
    # 26 is a long way below the FTM-400D's 47 - about -21 dB on this
    # codec's roughly 1 dB per step scale. Measured, not guessed: the
    # FT-7800R's data connector is far more sensitive on input.
    "ft7800r": {"label": "Yaesu FT-7800R", "speaker": 26, "pcm": 40,
                "controllable": False},
    "custom":  {"label": "Other", "controllable": None},
}

MIXER_CARD = os.environ.get("MIXER_CARD", "codec")
...
# The frequency goes in the location string because that is what other
# operators see in the EchoLink directory - it is how they know where to call.
# Derived from the placeholder frequency rather than written twice, so the two
# cannot drift apart. Three decimals is the usual convention there.
PLACEHOLDER_FREQ = "144.0000"
PLACEHOLDER = {
    "node_callsign": "MYCALL",
    "callsign": "MYCALL-L",
    "sysopname": "MyName",
    "location": f"[Svx] {float(PLACEHOLDER_FREQ):.3f}, MyTown",
    "password": "MyPass",
}
...
class HRI:
    ...
    def __init__(self, port):
        ...
        self.radio = None           # None when the radio does not identify
        self.serial_no = None
        self.firmware = None
    ...
    def detect_radio(self, attempts=8, gap=1.2):
        """Asks who the radio is. Returns its identity, or None.

        A controllable radio (FTM-400D and similar) needs several seconds
        before it answers: in the reference capture WIRES-X got nothing at
        t=1.0 s or t=2.1 s and only succeeded at t=4.1 s.

        A plain analogue radio never answers at all. A capture of WIRES-X with
        an FT-7800R shows D1V0000 sent 37 times over 54 seconds with no reply
        - and WIRES-X then never sends D1M either, because there is nothing to
        configure. That is not a fault: PTT, squelch and audio all work over
        the data connector regardless. Only the channel settings are the
        radio's own business.
        """
    ...
    def handshake(self, d1m, expect_controllable=None):
        ...
        # A radio declared analogue is still asked, just not for as long:
        # two attempts is enough to notice that someone swapped in a
        # controllable set, without spending ten seconds on every startup
        # waiting for a reply that is never coming.
        self.radio = self.detect_radio(
            attempts=2 if expect_controllable is False else 8)
        if self.radio:
            log(f"[OK] Radio: {self.radio}")
            # Not persistent - in node mode the radio is a slave and the host
            # owns the configuration, so this goes out on every connection.
            self.send(d1m)
            ...
            # Not fatal. The poll loop below is what carries PTT and squelch,
            # and it is already working - the box has been answering every
            # poll throughout the detection attempts.
            if expect_controllable is False:
                log("[OK] Radio does not identify itself, as expected for "
                    "this model. Set frequency, power and tone on the radio; "
                    "PTT, squelch and audio work normally.")
            else:
                log("[!] The radio does not identify itself, so the node "
                    "cannot set its frequency, power or tone.")
                log("    Set those on the radio itself. PTT, squelch and "
                    "audio work normally.")
                log("    If this radio should be controllable, it is in the "
                    "wrong mode: power it off and on holding [D/X] + [GM] "
                    "until the display reads HRI-200. [D/X] alone gives PDN "
...
class State:
    def __init__(self):
        ...
        self.radio = None
        self.controllable = False           # None when the radio does not identify
        self.serial_no = None
        self.firmware = None
        self.channel = ""
        self.squelch = False
...
def node_loop(state):
    ...
    while not state.stop.is_set():
        ...
        if hri is None or hri.ser is None:
            ...
            profile = RADIO_PROFILES.get(cfg.get("RADIO", "custom"), {})
            reason = hri.handshake(d1m, profile.get("controllable"))
            if reason:
                log(f"[FAIL] {reason}")
            ...
            state.set(connected=True, radio=hri.radio,
                      controllable=bool(hri.radio),
                      serial_no=hri.serial_no, firmware=hri.firmware,
                      channel=label if hri.radio else
                              "set on the radio - it does not identify itself",
                      error=None)
            sent_sql = None
        ...
        if state.reconfigure.is_set() and not hri.ptt:
            if not hri.radio:
                # Nothing to send it to. Clear the flag so the node does not
                # reconnect on a loop for a setting the radio cannot receive.
                state.reconfigure.clear()
                log("[!] Channel settings ignored: this radio does not "
                    "identify itself, so it cannot be configured remotely.")
                continue
...
def amixer(*args):
    """Runs amixer against the HRI-200 codec. Returns (ok, output)."""
    try:
        r = subprocess.run(["amixer", "-c", MIXER_CARD, *args],
...
def read_levels():
    ...
    Returns a dict with both keys, or None. Never a partial one: a card that
    exposes only one of the two controls, or words it differently, would
    otherwise hand back half an answer for callers to trip over.
    """
    ok, out = amixer("scontents")
    if not ok:
        return None
    levels = {}
    control = None
    for line in out.splitlines():
        ...
        if m:
            control = m.group(1)
        elif control in ("Speaker", "PCM"):
            m = re.search(r"(?:Playback|Capture) (\d+) \[", line)
            if m:
                levels["tx" if control == "Speaker" else "rx"] = int(m.group(1))
                control = None
    if "tx" not in levels or "rx" not in levels:
        if levels:
            log(f"[!] Mixer card '{MIXER_CARD}' exposes "
                f"{'Speaker' if 'tx' in levels else 'PCM'} but not the other "
                "control. Falling back to the configured levels.")
        return None
    return levels


def apply_levels(tx, rx):
    """Sets both levels and persists them. Returns a list of problems."""
    problems = []
    for name, value, limit in (("Speaker", tx, 47), ("PCM", rx, 55)):
        v = max(0, min(int(value), limit))
        ok, out = amixer("sset", name, str(v))
        if not ok:
            problems.append(f"{name}: {out.splitlines()[0] if out else 'failed'}")
    amixer("sset", "Bass Boost", "off")      # muddies transmit audio
    if not problems:
        # alsactl state is what survives a reboot; the mixer alone does not.
        try:
            subprocess.run(["sudo", "-n", "alsactl", "store"],
                           capture_output=True, timeout=15)
        except (OSError, subprocess.TimeoutExpired):
            problems.append("levels set but not saved - they will not survive "
                            "a reboot. Check the alsactl sudoers rule.")
    return problems
...
def make_app():
    ...
    def index():
        ...
        # What the card actually has right now, which may differ from the
        # config if someone ran amixer by hand.
        live = read_levels()
        if live:
            cfg = dict(cfg)
            cfg["TX_LEVEL"] = str(live.get("tx", cfg.get("TX_LEVEL")))
            cfg["RX_LEVEL"] = str(live.get("rx", cfg.get("RX_LEVEL")))
    ...
    def save():
        ...
        radio_model = f.get("radio", "custom")
        if radio_model not in RADIO_PROFILES:
            errors.append("Unknown radio model.")
            radio_model = "custom"
        try:
            tx_level = int(f.get("tx_level", 47))
            rx_level = int(f.get("rx_level", 45))
        except ValueError:
            errors.append("Audio levels must be whole numbers.")
            tx_level, rx_level = 47, 45
        if not 0 <= tx_level <= 47:
            errors.append("Transmit level must be between 0 and 47.")
        if not 0 <= rx_level <= 55:
            errors.append("Receive level must be between 0 and 55.")
        ...
        radio["RADIO"] = radio_model
        radio["TX_LEVEL"] = str(tx_level)
        radio["RX_LEVEL"] = str(rx_level)
        ...
        before_levels = read_levels() or {}
        level_problems = apply_levels(tx_level, rx_level)

        done = [f"Radio set to {label}."]
        if level_problems:
            done.append("Audio: " + "; ".join(level_problems))
        elif (before_levels.get("tx") != tx_level
              or before_levels.get("rx") != rx_level):
            done.append(f"Audio levels set to TX {tx_level}, RX {rx_level}.")
        if node_cs_changed:
...
def render(cfg, el, node_cs, notice, errors, token):
    ...
    current_radio = cfg.get("RADIO", "custom")
    radio_buttons = "".join(
        f'<label><input type="radio" name="radio" value="{k}"'
        f'{" checked" if k == current_radio else ""}'
        f' data-tx="{v.get("speaker", "")}" data-rx="{v.get("pcm", "")}"'
        f' data-controllable='
        f'"{"" if v.get("controllable") is None else int(v["controllable"])}">'
        f'<span>{e(v["label"])}</span></label>'
...
<style>
  ...
  * {{ box-sizing:border-box; }}
  ...
  .module.dimmed .row, .module.dimmed .check {{ opacity:.4;
                                                pointer-events:none; }}
  .module.dimmed .picker {{ opacity:1; pointer-events:auto; }}
  .notice {{ background:#2b2519; border-left:3px solid var(--lcd);
             padding:11px 14px; border-radius:3px; margin-bottom:16px;
             font-size:13px; color:var(--text); }}
  .notice b {{ color:var(--lcd); }}
  ...
  .picker label {{ flex:1 1 140px; }}
  .picker input {{ position:absolute; opacity:0; width:0; height:0; }}
  .picker span {{ display:block; text-align:center; padding:11px 8px;
                  background:#1b1e22; border:1px solid #454b54;
                  border-radius:3px; cursor:pointer; font-size:14px;
                  color:var(--legend); }}
  .picker input:checked + span {{ border-color:var(--steel);
                                  background:#22303a; color:var(--text);
                                  box-shadow:inset 0 0 0 1px var(--steel); }}
  .picker input:focus-visible + span {{ outline:2px solid var(--steel);
                                        outline-offset:2px; }}

  .slider {{ margin-bottom:16px; }}
  .slider .top {{ display:flex; justify-content:space-between;
                  align-items:baseline; margin-bottom:6px; }}
  .slider .top span {{ font-size:12px; color:var(--legend); }}
  .slider .val {{ font:600 13px ui-monospace,"DejaVu Sans Mono",monospace;
                  color:var(--lcd); font-variant-numeric:tabular-nums; }}
  .slider input[type=range] {{ width:100%; accent-color:var(--steel);
                               height:22px; }}
...
<div class="rack">
  ...
  <div class="module" id="radioModule">
    ...
    <div class="notice" id="dumbRadio" style="display:none">
      <b>This radio does not identify itself.</b> It cannot be tuned from here
      — set the frequency, power and tone on the radio. PTT, squelch and audio
      work normally, so the node is fully usable; only these controls are
      inert. A radio that should be controllable is in the wrong mode: power
  ...
  <div class="module">
    <p class="legend">Audio</p>

    <div class="slider">
      <div class="top"><span>Transmit level, out to the radio</span>
        <span class="val" id="txVal">{e(cfg.get('TX_LEVEL'))}</span></div>
      <input type="range" name="tx_level" id="tx_level" min="0" max="47"
             value="{e(cfg.get('TX_LEVEL'))}">
    </div>

    <div class="slider">
      <div class="top"><span>Receive level, in from the radio</span>
        <span class="val" id="rxVal">{e(cfg.get('RX_LEVEL'))}</span></div>
      <input type="range" name="rx_level" id="rx_level" min="0" max="55"
             value="{e(cfg.get('RX_LEVEL'))}">
    </div>

    <div class="hint">Choosing a radio above loads a starting point. Adjust by
      ear or by measurement — too low and DTMF stops decoding, too high and
      transmit audio distorts. These apply on save without restarting
      anything.</div>
  </div>
...
const DEF = {{
  freq:"{PLACEHOLDER_FREQ}", mode:"fm", tone:"none", power:"mid",
  ctcss:"88.5", dcs:"23", narrow:false,
  radio:"ftm400d", tx_level:"47", rx_level:"45",
...
function levels() {{
  $("txVal").textContent = $("tx_level").value;
  $("rxVal").textContent = $("rx_level").value;
}}

// Picking a radio loads its starting levels. Moving a slider afterwards means
// the levels are no longer that radio's, so the choice falls back to Custom -
// otherwise the button would claim a preset that is not in effect.
document.querySelectorAll('input[name=radio]').forEach(r => {{
  r.addEventListener("change", () => {{
    if (r.dataset.tx) $("tx_level").value = r.dataset.tx;
    if (r.dataset.rx) $("rx_level").value = r.dataset.rx;
    levels(); tuning();
  }});
}});

// Show straight away that a declared-analogue radio cannot be tuned, instead
// of waiting for the status poll to say what the choice already implies. The
// poll still has the last word, since it reports what the radio actually did.
function tuning() {{
  const sel = document.querySelector('input[name=radio]:checked');
  if (!sel || sel.dataset.controllable === "") return;
  const dumb = sel.dataset.controllable === "0";
  $("dumbRadio").style.display = dumb ? "" : "none";
  $("radioModule").className = dumb ? "module dimmed" : "module";
}}

["tx_level", "rx_level"].forEach(id => {{
  $(id).addEventListener("input", () => {{
    const sel = document.querySelector('input[name=radio]:checked');
    if (sel && sel.dataset.tx
        && ($("tx_level").value != sel.dataset.tx
         || $("rx_level").value != sel.dataset.rx)) {{
      const custom = document.querySelector('input[name=radio][value=custom]');
...
$("defaults").addEventListener("click", () => {{
  ...
  for (const [k,v] of Object.entries(DEF)) {{
    ...
    else if (el.length !== undefined && el[0] && el[0].type === "radio") {{
...
async function poll() {{
  try {{
    ...
    lamp("lRadio","Radio",
         s.connected ? (s.radio || "no ID") : "not connected",
         s.connected ? "on" : "bad");
    // Only say the settings are inert once we actually know - before the
    // first handshake completes, connected is false and we know nothing.
    const dumb = s.connected && !s.controllable;
    $("dumbRadio").style.display = dumb ? "" : "none";
    $("radioModule").className = dumb ? "module dimmed" : "module";
...
paint(); levels(); tuning(); poll(); setInterval(poll, 2000);
</script>
</body>
</html>"""
...
def do_check():
    ...
    live = read_levels()
    if live is None:
        report(False, "mixer levels readable",
               f"amixer cannot read Speaker and PCM on card '{MIXER_CARD}'. "
               "Is the box plugged in?")
    else:
        # 27/31 are the factory defaults: -20 dB on transmit, and low enough
        # on receive that DTMF decoding becomes unreliable.
        report(live["tx"] > 27 and live["rx"] > 31,
               f"mixer levels TX {live['tx']}/47, RX {live['rx']}/55",
               "These look like the factory defaults, which are far too quiet. "
               "Set them in the web panel, or with amixer.")
    ...
    print("\nEchoLink")
    el = read_keys(ECHOLINK_CONF, {"CALLSIGN", "PASSWORD", "LOCATION"})
    report(el.get("CALLSIGN") not in (None, PLACEHOLDER["callsign"]),
           "EchoLink callsign set")

    # The location string is what other operators read in the directory, so a
    # frequency left over from an earlier setting sends them to the wrong one.
    loc = el.get("LOCATION", "")
    m = re.search(r"(\d{2,3}[.,]\d{3,4})", loc)
    if m:
        try:
            shown = float(m.group(1).replace(",", "."))
            actual = float(str(cfg.get("FREQ", 0)).replace(",", "."))
            report(abs(shown - actual) < 0.0005,
                   f"EchoLink location agrees with the operating frequency",
                   f"The directory will show {shown:g} MHz but the node is on "
                   f"{actual:g}. Anyone reading it calls on the wrong "
                   "frequency. Update LOCATION in the web panel.")
        except ValueError:
            pass
    ...
    print("        (only applies to radios that identify themselves over the")
    print("         data connector. On a plain analogue set, tune the radio.)")
